...
/**
 * RealityValidator — coordinates knowledge retrieval, credibility scoring, and
 * contradiction detection to produce a single [RealityValidationResult].
 *
 * IRS-05 Contract Rules:
 *  - This is the ONLY class in `irs/reality/` that the orchestrator calls directly.
 *  - [CredibilityScorer] and [ContradictionDetector] are called internally; they do
 *    NOT interact with each other.
 *  - The result is fully traceable: all issues reference specific fields or facts.
 *  - Passes only when BOTH credibility is acceptable AND no contradictions exist.
...
class RealityValidator(
    ...
    private val credibilityScorer:   CredibilityScorer       = CredibilityScorer(),
     ...
     * @return [RealityValidationResult] capturing all credibility and contradiction findings.
     */
    fun validate(intent: IntentData): RealityValidationResult {
        val credibilityReport   = credibilityScorer.score(intent)
        val contradictionReport = contradictionDetector.detect(intent)
