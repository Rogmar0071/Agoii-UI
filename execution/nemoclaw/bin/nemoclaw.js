...
(async () => {
  ...
  // ── Agoii execution guard ─────────────────────────────────────────────────
  // The CLI must not execute runtime logic outside Agoii-controlled context.
  // Set AGOII_MODE=1 (or any non-empty value) to allow command execution.
  if (!process.env.AGOII_MODE) {
    process.stderr.write("  Error: AGOII_MODE is not set.\n");
    process.stderr.write("  The NemoClaw CLI can only execute under Agoii control.\n");
    process.stderr.write("  Set AGOII_MODE=1 to allow command execution.\n");
    process.exit(1);
  }
